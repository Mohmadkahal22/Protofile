...
<section style="padding:8rem 0 2.5rem;background:var(--dark-bg-2);position:relative;overflow:hidden;">
    <div class="glow-dot" style="width:500px;height:500px;background:var(--primary);top:-200px;left:-100px;"></div>
    ...
    <div class="container">
        ...
        <nav style="display:flex;align-items:center;gap:0.5rem;font-size:0.85rem;" data-aos="fade-up">
            ...
            <a href="{{ url('/') }}#services" style="color:var(--text-muted);text-decoration:none;transition:var(--transition-fast);" onmouseover="this.style.color='var(--primary)'" onmouseout="this.style.color='var(--text-muted)'">
                Services
            </a>
            <i class="fas fa-chevron-right" style="color:var(--text-muted);font-size:0.55rem;"></i>
...
<section class="section" style="background:var(--dark-bg);padding-top:3rem;">
    <div class="container" style="max-width:1200px;">
        ...
        <div id="service-not-found" style="display:none;text-align:center;padding:5rem 0;">
            ...
            <h3 style="font-size:1.5rem;font-weight:700;color:var(--text-primary);margin-bottom:0.75rem;">Service Not Found</h3>
...
/* Project cards (same as projects page) */
...
.svc-project-card-overlay .view-btn {
    ...
    padding: 0.45rem 0.9rem; background: rgba(43,155,255,0.2); border: 1px solid rgba(43,155,255,0.3);
    border-radius: var(--radius-sm); color: var(--primary); font-size: 0.78rem; font-weight: 600;
        ...
        .then(function(response) {
                ...
                imageHtml =
                    '<div class="svc-detail-image-wrap">' +
                        '<img src="' + imageUrl + '" alt="' + title + '" class="svc-detail-image" referrerpolicy="no-referrer" ' +
                            'onerror="this.parentElement.outerHTML=\'<div class=svc-detail-placeholder><i class=\\\'fas fa-cogs\\\' style=\\\'font-size:4rem;color:rgba(43,155,255,0.3);\\\'></i></div>\';">' +
                    '</div>';
            } else {
                imageHtml = '<div class="svc-detail-placeholder"><i class="fas fa-cogs" style="font-size:4rem;color:rgba(43,155,255,0.3);"></i></div>';
            }
            ...
            var statsHtml =
                '<div class="svc-detail-stats">' +
                    '<div class="svc-stat-item">' +
                        ...
                        '<div class="svc-stat-label">Project' + (projects.length !== 1 ? 's' : '') + '</div></div>' +
                    '</div>' +
                '</div>';
            ...
            if (projects.length > 0) {
                projectsHtml =
                    '<h2 class="svc-projects-header" data-aos="fade-up">' +
                        '<span class="icon-wrap"><i class="fas fa-briefcase" style="color:var(--primary);font-size:0.85rem;"></i></span>' +
                        'Projects in ' + title +
                    '</h2>' +
                    '<div class="grid-3">';
                ...
                projects.forEach(function(project, index) {
                    ...
                    projectsHtml +=
                        '<div class="card svc-project-card" data-aos="fade-up" data-aos-delay="' + (index % 3 * 80) + '" onclick="window.location.href=\'/project/' + (project.id || 0) + '\'">' +
                            '<div class="svc-project-card-img-wrap">' +
                                (projImageUrl
                                    ...
                                    : '<div class="svc-project-card-placeholder"><i class="fas fa-image" style="font-size:2.5rem;color:rgba(43,155,255,0.3);"></i></div>'
                                ) +
                            ...
                            '<div class="svc-project-card-body">' +
                                ...
                                '<p class="svc-project-card-desc">' + (project.description || 'No description available.') + '</p>' +
                            '</div>' +
                        '</div>';
                });
                ...
                projectsHtml =
                    '<div class="svc-no-projects" data-aos="fade-up">' +
                        '<div style="width:80px;height:80px;margin:0 auto 1.5rem;border-radius:50%;background:rgba(43,155,255,0.06);display:flex;align-items:center;justify-content:center;">' +
                            '<i class="fas fa-folder-open" style="font-size:2rem;color:var(--primary);opacity:0.5;"></i>' +
                        '</div>' +
                        '<p style="font-size:1.05rem;color:var(--text-secondary);font-weight:500;">No projects in this service yet</p>' +
                        '<p style="font-size:0.85rem;color:var(--text-muted);margin-top:0.5rem;">Projects will appear here as they are added.</p>' +
                    '</div>';
            }
            ...
            content.innerHTML =
                '<div class="svc-detail-grid" data-aos="fade-up">' +
                    '<div>' + imageHtml + '</div>' +
                    '<div>' +
                        '<h1 class="svc-detail-title">' + title + '</h1>' +
                        '<div class="svc-detail-desc">' +
                            (service.description || 'No description available.').replace(/\n/g, '<br>') +
                        '</div>' +
                        statsHtml +
                    '</div>' +
                '</div>' +
